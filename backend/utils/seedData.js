...
const seedData = async () => {
  try {
    ...
    const custodyStatuses = ["Police Custody", "Judicial Custody", "Not Arrested"]
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (Math.random() > 0.3) {
        ...
        const severityLevel = offenseType === "bailable" ? "low" : offenseType === "non-bailable" ? "high" : "medium"
        ...
        for (let j = 0; j < 4; j++) {
          similarCases.push({
            caseNumber: `BA-${Math.floor(Math.random() * 500) + 1}/${2022 - Math.floor(Math.random() * 3)}`,
      ...
      if (status !== "Pending") {
        ...
        for (let j = 0; j < hearingCount; j++) {
          const hearingDate = randomDate(filingDate, new Date(new Date().getTime() + 30 * 24 * 60 * 60 * 1000)) // Up to 30 days in future
          const hearingStatus = hearingDate < new Date() ? getRandomElement(["Completed", "Adjourned"]) : "Scheduled"

          const hearing = new Hearing({
            caseId: savedCase._id,
            date: hearingDate,
            time: `${Math.floor(Math.random() * 8) + 10}:${Math.random() > 0.5 ? "00" : "30"} ${Math.random() > 0.5 ? "AM" : "PM"}`,
            court,
            judge: judge._id,
            status: hearingStatus,
            purpose: "Bail hearing",
            notes: hearingStatus === "Completed" ? "Hearing completed as scheduled" : "",
            outcome:
              hearingStatus === "Completed"
                ...
                : "",
            attendees: [
              { user: applicant._id, role: "Applicant", attended: hearingStatus === "Completed" },
              { user: lawyer._id, role: "Lawyer", attended: hearingStatus === "Completed" },
              { user: judge._id, role: "Judge", attended: hearingStatus === "Completed" },
            ],
          })
